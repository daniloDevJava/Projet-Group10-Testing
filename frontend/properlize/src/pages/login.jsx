// Fichier : src/pages/Login.jsx (Version Corrigée et Complète)
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // On sauvegarde les informations reçues de l'API dans le localStorage.
      // Le test Playwright simule (mocke) cette `data`.
      if (data.name && data.accessToken) {
        localStorage.setItem('userLogin', data.name);
        localStorage.setItem('accessToken', data.accessToken);
        // Tu peux aussi sauvegarder le refreshToken si ton API le renvoie
        if (data.refreshToken) {
          localStorage.setItem('refreshToken', data.refreshToken);
        }
      } else {
        // Si l'API renvoie une réponse 200 mais sans les données attendues
        throw new Error("Réponse de l'API invalide");
      }
      
      alert('Connexion réussie !');
      // La navigation se fait après la sauvegarde
      navigate('/home');
        ...
        <div className="right-section">
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className="password-field">
              ...
              <span onClick={togglePassword} className="toggle-password" title={showPassword ? "Cacher le mot de passe" : "Afficher le mot de passe"}>
                {showPassword ? '🙈' : '👁️'}
              </span>
            </div>
